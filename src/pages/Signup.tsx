...
import { InstructorApplication } from '@/lib/mockData';
import { z } from 'zod';
...
const instructorApplicationSchema = z.object({
  reason: z.string().trim().min(10, { message: "Please provide at least 10 characters" }).max(500, { message: "Reason must be less than 500 characters" }),
  experience: z.string().trim().min(10, { message: "Please provide at least 10 characters" }).max(500, { message: "Experience must be less than 500 characters" }),
  expertise: z.string().trim().min(5, { message: "Please provide at least 5 characters" }).max(200, { message: "Expertise must be less than 200 characters" }),
  qualifications: z.string().trim().min(10, { message: "Please provide at least 10 characters" }).max(500, { message: "Qualifications must be less than 500 characters" }),
  courseIdea: z.string().trim().min(20, { message: "Please provide at least 20 characters" }).max(1000, { message: "Course idea must be less than 1000 characters" }),
});
...
export default function Signup() {
    ...
    if (success) {
      if (accountType === 'instructor') {
        ...
        if (newUser) {
          ...
          localStorage.setItem('instructorApplications', JSON.stringify(applications));
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <Card className="w-full max-w-2xl relative z-10 border-2 card-glass shadow-2xl max-h-[95vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                required
                className={`h-11 border-2 ${errors.password ? 'border-destructive' : ''}`}
              />
              {errors.password && <p className="text-xs text-destructive">{errors.password}</p>}
            </div>

            {accountType === 'instructor' && (
              <div className="space-y-4 p-5 rounded-xl bg-muted/50 border-2 border-primary/20">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    required
                    rows={3}
                    className={`border-2 ${errors.reason ? 'border-destructive' : ''}`}
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    required
                    rows={3}
                    className={`border-2 ${errors.experience ? 'border-destructive' : ''}`}
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    required
                    className={`h-11 border-2 ${errors.expertise ? 'border-destructive' : ''}`}
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    required
                    rows={3}
                    className={`border-2 ${errors.qualifications ? 'border-destructive' : ''}`}
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="courseIdea"
                    placeholder="Describe a course you'd like to create..."
                    value={courseIdea}
                    onChange={(e) => setCourseIdea(e.target.value)}
                    required
                    rows={4}
                    className={`border-2 ${errors.courseIdea ? 'border-destructive' : ''}`}
                  />
